...
The <a href={`${import.meta.env.BASE_URL}playground/snapshot`}>playground</a> renders this snapshot by default. Download <a href={`${import.meta.env.BASE_URL}playground/visualization_snapshot.json`}><code>visualization_snapshot.json</code></a> to inspect the source data.
