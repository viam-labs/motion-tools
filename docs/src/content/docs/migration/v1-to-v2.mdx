...
description: Move existing motion-tools visualizations from the removed client/client package to the client/api package without changing what they render.
---

import { Aside } from '@astrojs/starlight/components'

The [client/api](../../api/client-api/) package is the public API for drawing to the motion-tools visualizer. It replaces the legacy [client/client](https://github.com/viamrobotics/visualization/tree/v1.42.0/client/client) package, which has been removed. `v1.42.0` was the last release to ship it — pin to that tag if you need more time, then use this guide to move to v2 without changing what your visualizations render.
...
<Aside
	type="caution"
	title="Module path will change when v2 ships"
>
	The Go module is moving from `github.com/viam-labs/motion-tools` to
	`github.com/viamrobotics/visualization` when v2 is released. Both the `client/api` and `draw`
	packages will live under the new path, so plan on a one-time find-and-replace of your imports as
	part of the upgrade.
</Aside>
