...
2. **Assign arms and grippers** to the left and right controllers, and set per-controller motion **scale** and **rotation** behavior.
...
- **Plugins:** `<ControlWidgets />` (soft). In-headset camera feeds are driven by the camera widgets you've opened in `<ControlWidgets />`; without it no camera feeds appear, but the rest of XR works normally. See [Plugin dependencies](../dependencies/).
