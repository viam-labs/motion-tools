---
title: <Fullscreen />
description: Add a button that expands a motion-tools visualizer to cover the viewport.
---

`<Fullscreen />` adds a button to the top right of the visualizer that expands it to cover the browser viewport, above the host page's own content. Pressing <kbd>Escape</kbd> or clicking the button again returns the visualizer to its normal layout. While fullscreen, the host page's scroll is locked so only the visualizer receives it.
