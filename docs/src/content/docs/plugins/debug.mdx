---
title: <Debug />
description: Mount the TanStack Query devtools panel inside a motion-tools visualizer.
---

`<Debug />` surfaces the [TanStack Query devtools](https://tanstack.com/query/latest/docs/framework/svelte/devtools) panel — inspect the query cache, view in-flight mutations, and refetch manually — for any motion-tools instance. Motion-tools issues its Viam RPC calls through `@viamrobotics/svelte-sdk` (built on TanStack Query), so the devtools attaches to the `QueryClient` your `<ViamAppProvider>` already sets up. No extra wiring required.
