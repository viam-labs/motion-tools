...
description: Build a custom plugin for the motion-tools visualizer — a Svelte component you mount inside <Visualizer /> — for your own app or to contribute back.
---
...
>
	...
	to motion-tools. Only the **import paths** and the **packaging** differ. Where that matters, this
	guide splits into <b>In your app</b> and <b>In the repo</b> tabs.
</Aside>
...
```svelte
<script lang="ts">
	import { traits, useDetailsSection } from '@viamrobotics/motion-tools'
...
>
	Plugins contributed to motion-tools have access to the internal `useMouseRaycaster` hook
...
```svelte
<script lang="ts">
	import { useHotkey } from '@viamrobotics/motion-tools'
...
Workspace modes follow the same contribution shape. The visualizer has three (`monitor`, `build`, `move`), and the active one is persisted across sessions — so a plugin that owns a mode must declare it reachable while mounted, or a stored mode could name UI that no longer exists. `useEnvironmentMode` does exactly that: it registers in setup and releases on unmount, and a persisted mode nothing currently contributes resolves back to `monitor`.

```svelte
<script lang="ts">
	import { useEnvironment, useEnvironmentMode } from '@viamrobotics/motion-tools'
