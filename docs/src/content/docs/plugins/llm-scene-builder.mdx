...
<Aside type="caution">
`translation` values are **absolute** — they replace the current translation field directly. `orientation` values are **deltas** added to the current Euler angles.
</Aside>
...
The companion `<AISettings />` component (see below) stores an Anthropic API key in the browser under `settings.anthropicKey`. A minimal server-side route using the Anthropic SDK:
