import { TransformChangeType, type TransformWithUUID } from '@viamrobotics/sdk'
...
// The SDK hands the stream consumer plain objects with a `uuidString` alongside the
// message fields; spreading a generated message reproduces that shape.
const transformWithUUID = (
	uuidString: string,
	overrides: TransformInit = {}
): TransformWithUUID => ({
	...new Transform(overrides),
	uuidString,
})

const added = (uuid: string, overrides: TransformInit = {}): IncomingChange => ({
	changeType: TransformChangeType.ADDED,
	transform: transformWithUUID(uuid, overrides),
})
