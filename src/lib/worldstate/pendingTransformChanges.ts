...
/**
 * One stream event as the coalescer receives it, decoded from our own generated
 * `StreamTransformChangesResponse`. `uuid` is the transform's UUID bytes formatted as
 * a string, computed once here so no later stage touches the bytes. `updatedFields`
 * is the wire field mask; an UPDATED with no paths means the transform is full state.
 */
...
/** Keyed by UUID string. Insertion order is apply order, so parents stay ahead of children. */
