...
import { ClientNotConnectedError, type PartID, useRobotClient } from '../client/robot'
		...
		($client) =>
			({
				...
				mutationFn: async (id: string): Promise<void> => {
					if ($client === undefined) {
						throw new ClientNotConnectedError('Missing robot client when calling stopAll')
					}
