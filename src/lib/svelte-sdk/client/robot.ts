...
const createRobotClient = (): RobotClient => {
	const connectionStatus = writable<MachineConnectionEvent>(MachineConnectionEvent.DISCONNECTED)
...
export const useRobotClient = (partID: PartID) => {
	...
	// record.
	const clientWrapped = derived(context.clients, ($clients): RobotClient | undefined => {
		if (!partID) return
		return getIn($clients, [partID])
	})
