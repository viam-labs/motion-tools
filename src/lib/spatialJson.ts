 ...
 * and `GeometryConfig` offsets that reach the client both inside a motion plan's
 * `frame_system` and inside `FrameSystemConfig.kinematics`.
 *
 * These are hand conversions of Go this file cannot import
 * (`spatialmath/orientation_json.go`, `spatialmath/geometry.go`), so each switch
 * is a place the copy can fall behind its original without failing loudly.
 * Shared rather than duplicated because both readers of that JSON need the same
 * two subtleties — the encoding list, and which frame a geometry offset is
 * measured from.
 */
...
export interface FramePoseJson {
 ...
 *
 * Identity is correct for an absent or empty-string orientation — rdk's
 * `NoOrientationType` maps to `NewZeroOrientation` — and wrong for a `type` with
 * no case here, which is why only the latter warns: it renders confidently wrong
 * rather than visibly missing. Guessing a default encoding instead would be
 * worse than either, because `axis_angles` and both orientation-vector types
 * share the same `{ x, y, z, th }` field names: a wrong guess reads as a
 * plausible rotation, not as corrupt input.
 */
...
/**
 * A link's geometry offset is measured from the link's *parent*, sibling to the
 * link's own pose — but the renderer attaches geometry to the link and reads the
 * center as link-local. Passing it through unchanged doubles every offset, so
 * the frame's own pose has to be undone: `P_frame⁻¹ ∘ P_geometry`.
 *
 * This is rdk's convention, not a workaround. `FrameSystem.Transform` skips the
 * final transform when the subject is a `GeometriesInFrame`, explaining itself
 * in `referenceframe/frame_system.go` (~line 431): "A frame is assigned a pose
 * and a geometry and the two are not coupled together. This way you can define
 * everything relative to the parent frame."
 *
 * Only for frames that carry a model link's geometry. Obstacle geometries are
 * already local — the JSON looks identical either way, so only the frame's kind
