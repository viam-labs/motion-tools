...
func TestBurst(t *testing.T) {
	t.Run("DefaultsAppliedAndEchoed", func(t *testing.T) {
		s := newBurstTestStore(t, 3)

		resp, err := s.DoCommand(context.Background(), map[string]any{"command": "burst"})
		test.That(t, err, test.ShouldBeNil)
		test.That(t, resp["started"], test.ShouldEqual, true)
		test.That(t, resp["count"], test.ShouldEqual, burstDefaultCount)
		test.That(t, resp["period_ms"], test.ShouldEqual, burstDefaultPeriodMs)
		test.That(t, resp["ticks"], test.ShouldEqual, burstDefaultTicks)
		test.That(t, resp["transforms"], test.ShouldEqual, 3)
	})

	t.Run("RejectsOutOfRangeCount", func(t *testing.T) {
		s := newBurstTestStore(t, 3)

		_, err := s.DoCommand(context.Background(), map[string]any{"command": "burst", "count": 0.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "count")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinCount))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxCount))

		_, err = s.DoCommand(context.Background(), map[string]any{"command": "burst", "count": 10001.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "count")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinCount))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxCount))
	...
	t.Run("RejectsOutOfRangePeriodMs", func(t *testing.T) {
		s := newBurstTestStore(t, 3)

		_, err := s.DoCommand(context.Background(), map[string]any{"command": "burst", "period_ms": 9.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "period_ms")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinPeriodMs))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxPeriodMs))

		_, err = s.DoCommand(context.Background(), map[string]any{"command": "burst", "period_ms": 10001.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "period_ms")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinPeriodMs))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxPeriodMs))
	...
	t.Run("RejectsOutOfRangeTicks", func(t *testing.T) {
		s := newBurstTestStore(t, 3)

		_, err := s.DoCommand(context.Background(), map[string]any{"command": "burst", "ticks": 0.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "ticks")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinTicks))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxTicks))

		_, err = s.DoCommand(context.Background(), map[string]any{"command": "burst", "ticks": 100001.0})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "ticks")
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMinTicks))
		test.That(t, err.Error(), test.ShouldContainSubstring, fmt.Sprintf("%d", burstMaxTicks))
	...
	t.Run("RejectsNonNumericField", func(t *testing.T) {
		s := newBurstTestStore(t, 3)

		_, err := s.DoCommand(context.Background(), map[string]any{"command": "burst", "count": "many"})
		test.That(t, err, test.ShouldNotBeNil)
		test.That(t, err.Error(), test.ShouldContainSubstring, "count")
	})

	t.Run("RejectsZeroTransforms", func(t *testing.T) {
		s := newBurstTestStore(t, 0)

		_, err := s.DoCommand(context.Background(), map[string]any{"command": "burst"})
		test.That(t, err, test.ShouldNotBeNil)
	})
