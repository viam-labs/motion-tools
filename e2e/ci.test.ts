...
test('test', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'API key ID' }).click();
    await page.getByRole('textbox', { name: 'API key ID' }).fill('b3242cee-eade-4123-9633-5a1f2999920a');
    await page.getByRole('textbox', { name: 'API key value' }).dblclick();
    await page.getByRole('textbox', { name: 'API key value' }).fill('c4y8bry090n1ydijv40id3zttbzgag5n');
