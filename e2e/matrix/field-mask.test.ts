...
test.describe('field mask', () => {
	...
	test('moves a transform without resending its color', async ({ scene }) => {
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ present: true, color: RED_RGB })
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ worldPosition: [0.5, 0, 0], color: RED_RGB })
	})
	...
	test('recolors a transform without resending its pose', async ({ scene }) => {
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ worldPosition: [0.5, 0, 0] })
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ worldPosition: [0.5, 0, 0], color: { r: 0, g: 0, b: 1 } })
	})
	...
	test('reparents a transform without moving it', async ({ scene }) => {
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ parent: undefined, worldPosition: [0.2, 0, 0] })
		...
		await expect
			.poll(() => readEntityState(scene.page, BOX), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ parent: PARENT, orphan: undefined, worldPosition: [1.2, 0, 0] })
	})

	test('recolors a drawing without resending its opacity', async ({ scene }) => {
		const uuid = await scene.add(pointsDraft())
		await expect
			.poll(() => readEntityState(scene.page, POINTS), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ present: true, color: RED_RGB, opacity: 1 })
		...
		await expect
			.poll(() => readEntityState(scene.page, POINTS), { timeout: CONVERGE_TIMEOUT_MS })
			.toMatchObject({ color: { r: 0, g: 0, b: 1 }, opacity: 1 })
	})
