...
func handlePlanRequest(svc drawv1connect.DrawServiceHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			if foundReq && len(result.Trajectory) > 0 {
				break
			}
		}
		if !foundReq {
...
func handlePlanRequestStep(svc drawv1connect.DrawServiceHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// AddEntity is an upsert keyed on the deterministic SHA-1 UUID over
		// "name:parent", so re-rendering the same frames emits UPDATED events
