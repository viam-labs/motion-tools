...
type DrawPointCloudOptions struct {
	// ID is a stable identifier for the entity. When set, calling DrawPointCloud again with the same ID updates the existing entity in place. When empty, each call creates a new entity with a freshly generated UUID.
	ID string
	...
	// Colors controls how the cloud is colored. With no colors, the cloud's own per-point color data (if any) is used by the visualizer. Pass one color to override with a single shared color. Pass exactly PointCloud.Size() colors for per-point colors. Pass any other count to cycle through the slice as a palette.
	Colors []draw.Color
...
//
// Returns an error when Name is not ASCII printable or exceeds 100 characters, ErrVisualizerNotRunning if no visualizer is reachable, the underlying validation error if the cloud cannot be constructed (see draw.NewDrawnPointCloud, which rejects a negative downscaling threshold), or a wrapped RPC error if a network call fails.
